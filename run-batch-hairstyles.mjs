// Batch runner for hairstylesforseniors.com — generates N deep articles in one
// go, accumulating chosen topics (and seeding from the most recent existing
// titles) so the batch stays varied and avoids near-duplicates.
//
// Usage: node run-batch-hairstyles.mjs [N]   (default N = 4)

import dotenv from 'dotenv';
import fetch from 'node-fetch';
import { hairStylesConfig } from './functionsHairStyles.js';
import {
  pickTopicForSite,
  generateGlobalObj,
  generateImages,
...
async function recentTitles(limit = 40) {
  try {
    const url = `${STRAPI_API_URL}/api/${hairStylesConfig.collection}?fields[0]=title&sort=createdAt:desc&pagination[pageSize]=${limit}`;
...
async function makeOne(recent) {
  const cfg = { ...hairStylesConfig, recentTopicsHint: recent };
